...
if ($claimId) {
    ...
    if (!$claim) {
        // Return JSON for claim not found
        header('Content-Type: application/json');
        echo json_encode(['status' => 'error', 'message' => 'Claim not found.']);
        exit();
    }
} else {
    // Return JSON for no claim ID provided
    header('Content-Type: application/json');
    echo json_encode(['status' => 'error', 'message' => 'No claim ID provided.']);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action == "assign_assessor" && $assessor_id) {
        $response = $adminController->assignAssessor($claimId, $assessor_id);
        header('Content-Type: application/json');
        echo $response;
        exit();
    } else {
        $status = $action === 'approve' ? 'Approved' : 'Rejected';
        $adminController->updateClaimStatus($claimId, $status);

        // Return success response
        header('Content-Type: application/json');
        echo json_encode(['status' => 'success', 'message' => 'Claim status updated successfully.']);
        exit();
    }
}

// Fetch assessors for the page, only if it's not a POST request
                                        ...
                                        <div class="card">
                                            <div class="card-header d-flex justify-content-between ">      
												...
												<div class="claim-status">
													<?php if (!empty($claim['assessor_id'])): ?>
											...
											<div class="rounded d-flex flex-stack bg-active-lighten p-4" data-user-id="<?php echo htmlspecialchars($assessor['id']); ?>">
												...
												<div class="d-flex align-items-center">
													...
													<label class="form-check form-check-custom form-check-solid me-5">
														<input class="form-check-input" type="radio" name="assessor_id" value="<?php echo htmlspecialchars($assessor['id']); ?>" />
													</label>
													<!--end::Radio Button-->
													<!--begin::Avatar-->
